import { NotepadText } from 'lucide';
    ...
    for(let i = 0; i<localStorage.length; i++){
     const key = localStorage.key(i)
        if(key?.startsWith("note-")){
            ...
            tempDiv.innerHTML = content.split('<h1>')[1].split('</h1>')[0]
            .trim()
            .substring(0, 45)
            
            const tittle = tempDiv.textContent || 'nova nota'
...
export function updateNotesList(){
    ...
    for(let i = 0; i<notes.length; i++){
        ...
        noteElement.textContent = note.tittle
